/**
 * getUserByIdCommand - Get a user by ID
 * 
 * @package     @imajin/cli
...
export class GetUserByIdCommand extends BaseCommand {
    public readonly name = 'SimpleApi:getUserById';
    public readonly description = 'Get a user by ID';
    ...
    public async execute(args: any[], options: any): Promise<CommandResult> {
        try {
            ...
            const result = await this.service.getUserById(params, credentials);

            this.logSuccess('getUserById completed successfully', { result });

            return {
                success: true,
                data: result,
                message: 'getUserById completed successfully'
            };

        } catch (error) {
            const errorMessage = `getUserById failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
