...
export class CreateUserCommand extends BaseCommand {
    public readonly name = 'SimpleApi:createUser';
    public readonly description = 'Create a new user';
    ...
    public async execute(args: any[], options: any): Promise<CommandResult> {
        try {
            ...
            const result = await this.service.createUser(params, credentials);

            this.logSuccess('createUser completed successfully', { result });

            return {
                success: true,
                data: result,
                message: 'createUser completed successfully'
            };

        } catch (error) {
            const errorMessage = `createUser failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
