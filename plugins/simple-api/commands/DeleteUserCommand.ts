...
export class DeleteUserCommand extends BaseCommand {
    public readonly name = 'SimpleApi:deleteUser';
    public readonly description = 'Delete a user';
    ...
    public async execute(args: any[], options: any): Promise<CommandResult> {
        try {
            ...
            const result = await this.service.deleteUser(params, credentials);

            this.logSuccess('deleteUser completed successfully', { result });

            return {
                success: true,
                data: result,
                message: 'deleteUser completed successfully'
            };

        } catch (error) {
            const errorMessage = `deleteUser failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
