...
export class ServiceLayerProvider extends ServiceProvider {
    ...
    // NOTE: All console.log/error in these handlers are CLI output (user-facing interface)
    // These are command action handlers, not internal service logic
    /* eslint-disable no-console */

    private async handleListServices(options: any): Promise<void> {
        if (!this.serviceRegistry) {
            // CLI Output: User-facing error message (console is the UI)
        ...
        try {
            ...
            if (options.json) {
                // CLI Output: User-facing JSON (console is the UI)
